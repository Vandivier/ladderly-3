...
The repo root `AGENTS.md` is generated by `ladderly-io/scripts/python/create-copilot-instructions.py`. From the **repository root** (the directory that contains the `Makefile` and this `ladderly-io` folder), run:

```bash
make instructions
```

That is the usual shortcut; it uses `uv` under the hood. You can also run the script with `uv` directly—see `scripts/python/README.md`.
