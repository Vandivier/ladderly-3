...
// Create schema for editing a job post
...
type EditJobPostValues = z.infer<typeof EditJobPostSchema>
...
interface EditJobPostModalProps {
  jobPost: JobPostForCandidate
  onClose: () => void
  onSuccess: () => void
}

export const EditJobPostModal: React.FC<EditJobPostModalProps> = ({
  jobPost,
  onClose,
  onSuccess,
}) => {
  ...
  const { mutate: updateJobPost } = api.jobSearch.jobPost.update.useMutation({
    onSuccess: () => {
  ...
  const handleSubmit: FormProps<typeof EditJobPostSchema>['onSubmit'] = async (
    values,
  ) => {
